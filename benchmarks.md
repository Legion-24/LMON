# XDON Benchmarks: Compression & Token Efficiency

> **Status: numbers from the v1.0-beta.1 text format.** All measurements compare XDON/text to JSON. The planned BXDON binary encoding is a separate future layer and is not measured here.

## Overview

This document presents comprehensive benchmarking results comparing **XDON** (eXtensible Compact Object Notation) to **JSON** across multiple dimensions: byte size, character count, and token usage via real LLM tokenizers.

**Key Finding:** XDON achieves **20–28% token savings** depending on the LLM tokenizer, with improvements scaling to 35% on larger datasets.
...
**Insight:** Byte savings decrease as datasets grow (30.4% → 23.3%), because the header is amortized across more records. XDON's relative advantage is greatest on small, single-record structures.
...
- XDON's header-based design amortizes the schema across records
...
**Insight:** Claude's tokenizer is more efficient on JSON than GPT's (e.g., 79 tokens vs 21 tokens for small dataset), so the relative XDON advantage is smaller on single records. However, XDON still scales better—medium dataset shows 31.2% savings, matching GPT's medium performance.

**Implication:** XDON is most effective for batched, multi-record payloads (3+ records), regardless of tokenizer.
...
**Key Takeaway:** GPT tokenizer benefits 40% more from XDON on small datasets, but both converge on larger datasets (30%+ savings).
...
- **Output tokens:** $15.00 per 1M tokens (if XDON is used for LLM output)
...
- **Output tokens:** $25.00 per 1M tokens (if XDON is used for LLM output)
...
- **OpenAI:** ~$1,865 saved annually (28.3% XDON efficiency)
- **Anthropic:** ~$2,205 saved annually (20.1% XDON efficiency)
...
3. **Human readability:** JSON is more familiar; XDON has a learning curve
4. **Existing ecosystems:** Converting from JSON requires migration effort

### Tokenizer-Specific Notes

- **GPT tokenizers (tiktoken):** ~28% savings; benefits most from XDON
...
All XDON files round-trip correctly:
- Parse XDON → JavaScript/Python object
- Stringify object → XDON
- Output byte-matches input (lossless)

JSON structures are semantically equivalent to XDON; type inference verified:
...
|--------|--------|-----------------|
| **Byte size savings** | 23–30% | XDON is 25% more compact |
| **Token savings (GPT)** | 19–35% | XDON is 28% more efficient for GPT models |
| **Token savings (Claude)** | 8–31% | XDON is 20% more efficient for Claude |
...
**Recommendation:** XDON is production-ready for structured, repeated-schema workloads. Deploy where:
...
XDON macros enable text reuse and dynamic content. There are three strategies:

1. **No macros** — all content literal (baseline)
2. **Macros in document** — definitions included in XDON text
...
1. **Macros in document add overhead** — Defining macros in XDON increases input size by 39.2%. This overhead is recovered if the same template is reused many times.

2. **Preloaded macros are nearly free** — Using `initialContext` adds only 2.1% overhead (3 bytes for the `%admin` and `%user` references). The macro definitions are in your code, not the XDON text.
...
## JSON vs XDON with Macros

How do macros affect XDON's token savings vs JSON?
...
|--------|-------------|--------------|--------------|-----------------|
| **JSON** | 280 | 30 | — | — |
| **XDON (no macros)** | 143 | 20 | 143 | **48.9% bytes, 33.3% tokens** |
| **XDON (macros in doc)** | 199 | 29 | 144 | 28.9% bytes (input), 48.6% (output) |
| **XDON (preloaded)** | 146 | 20 | 143 | **47.9% bytes, 33.3% tokens** |

### Key Findings

1. **Preloaded macros maintain full token savings** — 33.3% token reduction vs JSON, same as XDON without macros

2. **Preloaded macros add minimal byte overhead** — Only 3 bytes (2.1%) vs XDON baseline, compared to 56 bytes (39.2%) for macros in document

3. **Best of both worlds** — Preloaded macros preserve the ~48% byte savings while keeping the XDON text clean and readable (no macro definition boilerplate)

4. **Output equivalence after expansion** — All XDON variants produce identical output (144 bytes after macro expansion)

### When to Use Macros vs Plain XDON

| Scenario | Recommendation | Why |
|----------|----------------|-----|
| Template sent once | Plain XDON (no macros) | Simplest; no overhead |
...
- **XDON without macros:** 48.9% byte savings, 33.3% token savings vs JSON
- **XDON with preloaded macros:** 47.9% byte savings, 33.3% token savings vs JSON (essentially identical)
...
2.1% overhead vs plain XDON. Macros in document add 39.2% overhead (input bytes).
```
...
For **complex nested objects** (user profiles, order records, etc.), XDON with preloaded headers shines. Single records save little; batches save huge amounts.
...
| JSON | 495 | 38 | — |
| XDON (no macros) | 331 | 38 | 33.1% bytes, 0% tokens |
| **Batch of 5 records** | — | — | — |
| JSON | 2,435 | 166 | — |
| XDON (header inline) | 897 | 102 | 63.2% bytes, 38.6% tokens |
| XDON (header preloaded) | **776 input** | **87** | **68.1% bytes, 47.6% tokens** ✓ |

### Per-Record Analysis

| Metric | JSON | XDON w/ Preload |
|--------|------|-----------------|
...
| XDON (inline header × 10) | 8,970 | 63.2% |
| XDON (header preloaded, 1 call) | **7,883** | **67.6%** ✓ |

**Additional savings with preload: 12.1% of XDON size (1,087 bytes saved over 10 calls)**
...
**XDON Version:** 1.0.0 (SPEC-aligned)  
**Repository:** [Legion24 XDON](https://github.com/legion24/xdon)
