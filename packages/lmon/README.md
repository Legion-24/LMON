# @legion24/lmon — JavaScript/TypeScript

LMON parser and stringifier for Node.js and browsers. Includes optional macro preprocessing.
...
```typescript
import { stringify } from '@legion24/lmon';
...
```typescript
import { expand, parse } from '@legion24/lmon';
...
Parse LMON and return a JavaScript object.
...
Convert a JavaScript object to LMON.
...
Parse LMON and return the raw AST (for advanced use).

```typescript
parseToAST(input: string): LMONDocument;
```
...
```typescript
import { LMONParseError, LMONMacroError } from '@legion24/lmon';

try {
  const data = parse(malformed);
} catch (err) {
  if (err instanceof LMONParseError) {
...
try {
  ...
  if (err instanceof LMONMacroError) {
...
LMON is a token-efficient alternative to JSON. It defines structure once in a header, then repeats only data:
...
| Package | Language |
|---------|----------|
| `@legion24/lmon` | JavaScript / TypeScript (this) |
| `lmon` | Python |
