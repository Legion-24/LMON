...
import { XDONParseError } from './errors.js';
...
class Parser {
  ...
  private parseDirective(): string {
    ...
    // Format: !XDON MAJOR.MINOR
    const m = /^!XDON\s+(\d+)\.(\d+)\s*$/.exec(directive);
    if (!m) {
      throw new XDONParseError(
    ...
    if (major !== SUPPORTED_VERSION_MAJOR) {
      throw new XDONParseError(
        `Unsupported XDON version '${m[1]}.${m[2]}' — this parser supports v${SUPPORTED_VERSION_MAJOR}.x`,
        tok.line,
        tok.column,
      );
    }
...
export function parseToAST(input: string, options?: ParseOptions): XDONDocument {
...
function evaluate(doc: XDONDocument): Record<string, unknown> | unknown[] {
