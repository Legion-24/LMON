...
import { XDONStringifyError } from './errors.js';
...
function inferSchema(obj: Record<string, unknown>, seen: WeakSet<object> = new WeakSet()): SchemaField[] {
  if (seen.has(obj)) {
    throw new XDONStringifyError('Cyclic reference in input');
  }
  seen.add(obj);
...
function formatScalar(value: unknown, seen: WeakSet<object>): string {
  ...
  throw new XDONStringifyError(`Unsupported value type: ${typeof value}`);
}
