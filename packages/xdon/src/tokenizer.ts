...
/**
 * Tokenize XDON text into a stream of tokens. Skips a leading UTF-8 BOM.
 * Reserved leading characters (@, #, !, %) cause a parse error in bare position.
 * Directives (lines starting with `!XDON`) are emitted as DIRECTIVE tokens.
 */
export function tokenize(input: string): Token[] {
  ...
  const tryReadDirective = (): boolean => {
    ...
    // Look ahead for "XDON " or end-of-token directive name.
