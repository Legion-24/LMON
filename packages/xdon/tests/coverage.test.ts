...
describe('coverage: tokenizer', () => {
  ...
  test('unterminated quoted string', () => {
    expect(() => parse('{"unclosed')).toThrow(XDONParseError);
  });

  test('newline inside quoted string is rejected', () => {
    expect(() => parse('{"line\nbreak"}')).toThrow(XDONParseError);
  });

  test('unterminated escape in quoted string', () => {
    expect(() => parse('{"trail\\')).toThrow(XDONParseError);
  });

  test('unterminated escape in bare value', () => {
    expect(() => parse('{abc\\')).toThrow(XDONParseError);
  });
  ...
  test('unexpected character errors', () => {
    expect(() => parse('{?}')).toThrow(XDONParseError);
  });
...
describe('coverage: parser', () => {
  ...
  test('document EOF before closing brace', () => {
    expect(() => parse('{1,2')).toThrow(XDONParseError);
  });
  ...
  test('unexpected EOF in field value position', () => {
    expect(() => parseToAST('{')).toThrow(XDONParseError);
  });

  test('missing row body after label', () => {
    expect(() => parse('label:')).toThrow(XDONParseError);
  });

  test('label expected but got non-label-token', () => {
    expect(() => parse('(,b)\n{1,2}')).toThrow(XDONParseError);
  });
  ...
  test('depth limit triggers at array depth too', () => {
    ...
    for (let k = 0; k < 50; k++) s += ']';
    s += '}';
    expect(() => parse(s, { maxDepth: 5 })).toThrow(XDONParseError);
  });
  ...
  test('expression unexpected token after parsing', () => {
    ...
    expect(() => expand('%{1 5}')).toThrow(XDONMacroError);
  });
});
...
describe('coverage: stringifier', () => {
  ...
  test('schema-merged array slot rejects scalar in earlier row', () => {
    ...
    expect(() => stringify(data)).toThrow(XDONStringifyError);
  });
  ...
  test('unsupported value type throws', () => {
    const sym = Symbol('x');
    expect(() => stringify([sym as unknown])).toThrow(XDONStringifyError);
  });
  ...
  test('schemaless cycle detection in formatRowSchemaless', () => {
    const a: any = { x: 1 };
    a.self = a;
    expect(() => stringify({ row: [a, 'plain'] })).toThrow(XDONStringifyError);
  });
  ...
  test('schemaless cycle detection in formatScalar nested object', () => {
    ...
    expect(() => stringify({ k: 'string', mixed: a })).toThrow(XDONStringifyError);
  });
...
describe('coverage: macros', () => {
  test('expression with unbalanced parens', () => {
    expect(() => expand('%{(1+2}')).toThrow(XDONMacroError);
  });

  test('expression with unexpected character', () => {
    expect(() => expand('%{1+a}')).toThrow(XDONMacroError);
  });

  test('expression invalid leading op', () => {
    expect(() => expand('%{*5}')).toThrow(XDONMacroError);
  });

  test('expression: trailing operator (number expected)', () => {
    expect(() => expand('%{1+}')).toThrow(XDONMacroError);
  });

  test('division by zero', () => {
    expect(() => expand('%{1/0}')).toThrow(XDONMacroError);
  });
  ...
  test('macro takes no params but called with args', () => {
    expect(() => expand('%x = "v"\n%x(arg)')).toThrow(XDONMacroError);
  });

  test('macro arg count mismatch', () => {
    expect(() => expand('%f(a,b) = "x"\n%f(1)')).toThrow(XDONMacroError);
  });

  test('unclosed parameter list', () => {
    expect(() => expand('%f(a) = "x"\n%f(arg')).toThrow(XDONMacroError);
  });
  ...
  test('expansion depth exceeded errors', () => {
    expect(() => expand('%a = "%a"\n%a', { maxDepth: 3 })).toThrow(XDONMacroError);
  });
...
describe('coverage: json bridge', () => {
  ...
  test('fromJSON parses JSON and stringifies XDON', () => {
