...
const FIXTURES: Fixture[] = [
  ...
  {
    name: 'types',
    xdon: '(string,integer,float,boolean,null_val)\nexample:{hello,42,3.14,true,null}\n',
    json: {
...
describe('Round-trip Integration Tests', () => {
  FIXTURES.forEach(({ name, xdon, json }) => {
    it(`should parse fixture to expected JSON: ${name}`, () => {
      expect(parse(xdon)).toEqual(json);
    });
