...
from .errors import XDONParseError
...
_VERSION_DIRECTIVE_RE = re.compile(r"^!XDON\s+(\d+)\.(\d+)\s*$")
_SUPPORTED_MAJOR = 1
...
class Parser:
    """Recursive descent parser for XDON."""
    ...
    def parse(self) -> XDONDocument:
        ...
        return XDONDocument(header=header, body=body, version_directive=version_directive)
    ...
    def parse_directive(self) -> str:
        ...
        if major != _SUPPORTED_MAJOR:
            raise XDONParseError(
                f"Unsupported XDON version '{m.group(1)}.{m.group(2)}' — "
...
def parse_to_ast(input_text: str, options: Optional[ParseOptions] = None) -> XDONDocument:
    """Parse XDON text to AST."""
...
def parse(
    ...
    """Parse XDON text to a Python object."""
    ast = parse_to_ast(input_text, options)
    return evaluate(ast)


def evaluate(doc: XDONDocument) -> Union[Dict[str, Any], List[Any]]:
