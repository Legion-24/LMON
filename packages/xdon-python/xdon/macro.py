...
from .errors import XDONMacroError
...
class ExprEvaluator:
    ...
    def _lex(self, expr: str, line: int, col: int) -> List[ExprToken]:
        ...
        while i < len(expr):
            ...
            if ch == "+":
                ...
                raise XDONMacroError(f"Unexpected character in expression: '{ch}'", line, col)
    ...
    def _expect(self, t: ExprTokenType, line: int, col: int) -> ExprToken:
        cur = self._cur()
        if cur.type != t:
            raise XDONMacroError(f"Expected {t} but got {cur.type}", line, col)
        return self._adv()
    ...
    def _parse_mul(self, line: int, col: int) -> float:
        ...
        while self._cur().type in (ExprTokenType.STAR, ExprTokenType.SLASH, ExprTokenType.PERCENT):
            ...
            if op.type == ExprTokenType.STAR:
                ...
                if right == 0:
                    raise XDONMacroError("Division by zero in expression", line, col)
                left = left / right
            else:
                if right == 0:
                    raise XDONMacroError("Modulo by zero in expression", line, col)
                left = left % right
        return left
...
def _expand_line(
    ...
    if depth > state.max_depth:
        raise XDONMacroError("Macro expansion depth exceeded", line_num, 1)
