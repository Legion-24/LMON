...
from .errors import XDONStringifyError
...
def stringify(obj: Any, row_labels: bool = True) -> str:
    """Convert a Python object to XDON text."""
    seen: Set[int] = set()
...
def _stringify_object(obj: Dict[str, Any], row_labels: bool, seen: Set[int]) -> str:
    if id(obj) in seen:
        raise XDONStringifyError("Cyclic reference in input")
    seen.add(id(obj))
...
def _format_scalar(value: Any, seen: Set[int]) -> str:
    ...
    raise XDONStringifyError(f"Unsupported value type: {type(value).__name__}")
