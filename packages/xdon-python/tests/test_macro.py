import pytest
from xdon import expand, MacroContext, MacroDefinition, ExpandOptions, XDONMacroError, parse
...
class TestSimpleVariableMacros:
    ...
    def test_a4_forward_ref_error(self):
        input_text = '%x\n%x = "a"'
        with pytest.raises(XDONMacroError):
            expand(input_text)

    def test_a5_embedded_in_xdon(self):
...
class TestIntegrationWithXDONParsing:
...
class TestSpecDefinedMacros:
    ...
    def test_env_reads_when_allowlisted(self):
        from xdon import ExpandOptions
        import os
        os.environ['TEST_VAR_MACRO'] = 'test_value_123'
