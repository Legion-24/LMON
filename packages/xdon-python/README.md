...
XDON parser and stringifier for Python. Includes optional macro preprocessing.
...
```python
from xdon import parse

xdon_text = """(name,age,active)
alice:{Alice,30,true}
bob:{Bob,25,false}"""

result = parse(xdon_text)
# {
...
### Stringifying to XDON

```python
from xdon import stringify
...
expanded = expand(xdon_with_macros)
data = parse(expanded)
```
...
```python
from xdon import to_json, from_json

# XDON to JSON
xdon_str = '(name)\nalice:{Alice}'
json_str = to_json(xdon_str)

# JSON to XDON
json_str = '{"user1": {"name": "Alice"}}'
xdon_str = from_json(json_str)
```
...
Parse XDON text to Python object.

```python
from xdon import parse

data = parse(xdon_text)
```

#### `parse_to_ast(input: str) -> XDONDocument`

Parse XDON text to AST (for advanced usage).

### Stringification

#### `stringify(data: Any, options: Optional[StringifyOptions] = None) -> str`

Convert Python object to XDON text.
...
expanded = expand(xdon_with_macros, ExpandOptions(strict=True, max_depth=16))
```
...
```python
from xdon import parse, expand
from xdon.errors import XDONParseError, XDONMacroError

try:
    data = parse(malformed)
except XDONParseError as err:
    print(f"Parse error at {err.line}:{err.column}: {err.message}")

try:
    expanded = expand(xdon_with_macros)
except XDONMacroError as err:
    print(f"Macro error at {err.line}:{err.column}: {err.message}")
```

### XDONParseError

Raised on XDON syntax errors with line/column information.
...
```bash
pytest tests/
pytest tests/ --cov=xdon
```

## See Also

- [SPEC.md](../../SPEC.md) - Formal XDON specification
- [JS/TS implementation](../xdon/) - TypeScript reference implementation
